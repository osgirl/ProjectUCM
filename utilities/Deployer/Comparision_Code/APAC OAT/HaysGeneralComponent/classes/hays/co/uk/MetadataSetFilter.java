...
public class MetadataSetFilter implements FilterImplementor {
	...
	public int doFilter(Workspace ws, DataBinder db, ExecutionContext ctx) throws DataException, ServiceException {
		...
		if("validateStandard".equals(filterParam) ) {
			...
			//Update value of SolrFeedRequired metadata to Yes
			updateSorlRequiredField();
			
		}
		if("preComputeDocName".equals(filterParam) ) {
			//Editing Content ID of Useful Links Template
			updateUsefulLinksContentId();
		}
		return CONTINUE;
	}
	 ...
	 * Update value of SolrFeedRequired metadata to Yes
	 */
	private void updateSorlRequiredField() 
	{
		m_binder.putLocal("xSolrFeedRequired", "Yes");
	}
...
//////////////added for r7 start///////////////////////////////////////////////////////
	 ...
	 * Editing Content ID of Useful Links Template
	 */
	private void updateUsefulLinksContentId() 
	{
		String idcService = m_binder.getLocal("IdcService");
		String locale = m_binder.getLocal("xLocale");
		String xRegionDefinition = m_binder.getLocal("xRegionDefinition");
		int revision = Integer.parseInt(m_binder.getLocal("dRevLabel"));
		String contentId= m_binder.getLocal("dDocName");
		SystemUtils.trace("hays_useful_links", "Document Revision: " + revision);
		SystemUtils.trace("hays_useful_links", "Document Region Definition: " + xRegionDefinition);
		SystemUtils.trace("hays_useful_links", "Document IDC Service: " + idcService);
		
		if(idcService!=null && idcService.equalsIgnoreCase("SS_CHECKIN_NEW")
				&& xRegionDefinition!=null && xRegionDefinition.equalsIgnoreCase("RGDEF_USEFUL_LINKS")
				/*&& revision==1*/)
		{
			
			SystemUtils.trace("hays_useful_links", "Old contentId: " + contentId);
			contentId = "USEFULLINKS_"+locale;
			m_binder.putLocal("dDocName",contentId);
			SystemUtils.trace("hays_useful_links", "New contentId: " + contentId);
		}
