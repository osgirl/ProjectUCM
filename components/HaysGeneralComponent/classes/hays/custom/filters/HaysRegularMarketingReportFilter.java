...
public class HaysRegularMarketingReportFilter implements FilterImplementor
{

	public final static String TRACE_NAME = "HaysRegularMarketingReportFilter";
	...
	public int doFilter(Workspace ws, DataBinder binder, ExecutionContext cxt) throws DataException, ServiceException
	{
		...
		if (action.equals("MarketingReportMonthlyEvent"))
		{
			// you MUST perform at least one update
			update(action, "event starting...", ws);

			binder.putLocal("IdcService", "HAYS_REGULAR_MARKETING_REPORT");
			executeService(binder, "sysadmin", true);
			SystemUtils.trace(TRACE_NAME, "Executed service :: HAYS_REGULAR_MARKETING_REPORT");
