...
public class MetadataSetFilter1 implements FilterImplementor {
	...
	private static final String ExecuteDescriptiveUrl = SharedObjects.getEnvironmentValue("ExecuteDescriptiveUrl");
...
//	Code Starts for Descriptive Url
	private void populateDescriptiveUrl(){
		  ...
		  String ContentType = ","+dDocType+",";
		  String ExecuteDescriptiveUrlFinal = ","+ExecuteDescriptiveUrl+",";
		  SystemUtils.trace("hays_search", "Content Type is : ="+dDocType);
		  if (dDocType != null &&  (ContentType.indexOf(ExecuteDescriptiveUrlFinal)>0)) 
					{
			...
			if ((pageUrl!=null && pageUrl.length()>0) ){
					...
					String dRevClassID=this.m_binder.getLocal("dRevClassID");

					SystemUtils.trace("DescriptiveUrl", "DescriptiveUrl+dRevClassID --  : " + pageUrl+"-"+dRevClassID);
				try {					 
					m_binder.putLocal("xLocalerecordID", pageUrl+"-"+dRevClassID);					
				}
				...
				ex.getStackTrace();
					...
					String dRevClassID=this.m_binder.getLocal("dRevClassID");
					try {					 
						m_binder.putLocal("xLocalerecordID", urlDocTitle+"-"+dRevClassID);					
					}
	...
	private void deleteSpecialismFromOfficeTable()
	{
		  ...
		  SystemUtils.trace("hays_search", "Content Type is : ="+dDocType);
			...
			if (dDocType != null && (dDocType.equalsIgnoreCase("Office")) && officeId != null && !("".equalsIgnoreCase(officeId))) //Execute only during an updation 
					{
				try {
